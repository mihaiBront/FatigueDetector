...
class YawnDetection(Detector):
    def __init__(self):
        ...
        self.threshold = 1.2  # Aumentar el umbral para ser más estricto
    ...
    def check_open_mouth(self, mouth_distances: dict) -> bool:
        ...
        # Solo considerar la boca abierta si la distancia entre los labios es significativamente mayor
        if lips_distance > (chin_distance * self.threshold):
            self.open_mouth = True
        else:
            self.open_mouth = False
            
        print(f"Distancia labios: {lips_distance:.2f}, Distancia barbilla: {chin_distance:.2f}, Ratio: {lips_distance/chin_distance:.2f}")
